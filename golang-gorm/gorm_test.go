...
func TestUpdate(t *testing.T) {
	...
	user.Name.FirstName = "Alam"
	user.Name.MiddleName = ""
	user.Name.LastName = "Hasnur"
...
func TestUpdateSelectedColumns(t *testing.T) {
	err := db.Model(&User{}).Where("id = ?", "10").Updates(map[string]interface{}{
		"middle_name": "Nur",
		"last_name":   "Wahid",
	}).Error
	assert.Nil(t, err)

	err = db.Model(&User{}).Where("id = ?", "10").Update("password", "diubahlagi").Error
	assert.Nil(t, err)
...
func TestLock(t *testing.T) {
	err := db.Transaction(func(tx *gorm.DB) error {
		var user User
		err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Take(&user, "id = ?", "11").Error
		if err != nil {
			return err
		}
...
func TestPreloadManyToMany(t *testing.T) {
	var product Product
	err := db.Preload("LikedByUser").First(&product, "id = ?", "P001").Error
	assert.Nil(t, err)
	assert.Equal(t, 2, len(product.LikedByUser))
}
...
func TestAssociationFind(t *testing.T) {
	...
	var users []User
	err = db.Model(&product).Where("users.first_name LIKE ?", "User%").Association("LikedByUser").Find(&users)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(users))
}
...
func TestAssociationAppend(t *testing.T) {
	...
	err = db.Model(&product).Association("LikedByUser").Append(&user)
	assert.Nil(t, err)
}
...
func TestAssociationDelete(t *testing.T) {
	...
	err = db.Model(&product).Association("LikedByUser").Delete(&user)
	assert.Nil(t, err)
}
...
func TestAssociationClear(t *testing.T) {
	...
	err = db.Model(&product).Association("LikedByUser").Clear()
	assert.Nil(t, err)
}
